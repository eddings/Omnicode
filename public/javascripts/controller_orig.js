			...
			if (stackFs) {
				stackFs.forEach((f, j) => {
					...
					encodedLocalNames.forEach((name, k) => {
						var obj = {};
						if ((f['encoded_locals'][name] !== null) && !isNaN(f['encoded_locals'][name])) {
							if (name === "__return__") {
	...
	sendRunRequestAndVisualize(URL, dataObj, mode, checkpoint_idx, testcase_idx) {
		$.post({
			...
			success: (data) => {
				...
				$("#checkpoint-status" + checkpoint_idx).html("Done");
				setTimeout(() => {
					this.editorStatus.fadeOut();
					$("#checkpoint-status" + checkpoint_idx).fadeOut();
				}, 2000);
